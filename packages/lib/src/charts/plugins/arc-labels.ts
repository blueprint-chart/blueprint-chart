...
function pushForward(ys: number[], gap: number): void {
...
function pushBackward(ys: number[], gap: number): void {
...
interface SideDatum extends ArcLabelDatum {
  midAngle: number
}

function splitIntoSides(data: ArcLabelDatum[]): { left: SideDatum[], right: SideDatum[] } {
  const left: SideDatum[] = []
  const right: SideDatum[] = []
...
function renderLabelLeader(
  g: d3.Selection<SVGGElement, unknown, null, undefined>,
  arcX: number, arcY: number, stubX: number, stubY: number, hEndX: number,
): void {
  g.append('polyline')
...
function renderLabelText(
  g: d3.Selection<SVGGElement, unknown, null, undefined>,
  p: ArcLabelDatum, textX: number, stubY: number,
  isRight: boolean, fontSize: number, bgColor: string,
  showLabel: boolean, showValue: boolean,
): void {
  ...
  if (showValue) {
    appendValueTspan(text, p, textX, showLabel ? '1.2em' : '0.35em', '#666')
  }
}

function appendValueTspan(
  text: d3.Selection<SVGTextElement, unknown, null, undefined>,
  p: ArcLabelDatum, x: number, dy: string, fill: string, opacity?: number,
): void {
  ...
  const tspan = text.append('tspan')
    ...
    .text(valueText)
  if (opacity != null) {
    tspan.attr('opacity', opacity)
  }
}

function renderSingleLabel(
  g: d3.Selection<SVGGElement, unknown, null, undefined>,
  p: SideDatum, labelAng: number, anchorR: number, stubR: number,
  isRight: boolean, fontSize: number, bgColor: string,
): void {
  ...
  renderLabelLeader(g, arcX, arcY, stubX, stubY, hEndX)

  const textX = hEndX + xSign * 4
  renderLabelText(g, p, textX, stubY, isRight, fontSize, bgColor, showLabel, showValue)
}

function renderSide(
  parent: d3.Selection<SVGGElement, unknown, null, undefined>,
  side: SideDatum[], isRight: boolean,
  anchorR: number, stubR: number, minAngGap: number,
  fontSize: number, bgColor: string,
): void {
  if (side.length === 0) {
    return
  }
  ...
  side.forEach((p, i) => {
    renderSingleLabel(g, p, resolvedAngs[i], anchorR, stubR, isRight, fontSize, bgColor)
  })
}
...
export function renderArcLabels(
  ...
  const { left, right } = splitIntoSides(data)

  renderSide(parent, right, true, anchorR, stubR, minAngGap, fontSize, bgColor)
  renderSide(parent, left, false, anchorR, stubR, minAngGap, fontSize, bgColor)
}

// Minimum angular span (radians) for a slice to receive an inside label
const MIN_INSIDE_ANGLE = 0.3

function createInsideTextElement(
  g: d3.Selection<SVGGElement, unknown, null, undefined>,
...
function appendLabelTspan(
  text: d3.Selection<SVGTextElement, unknown, null, undefined>,
  cx: number, label: string, showValue: boolean, fill: string,
): void {
  text.append('tspan')
    .attr('x', cx)
...
function renderInsideLabel(
  g: d3.Selection<SVGGElement, unknown, null, undefined>,
  d: ArcLabelDatum, centroidR: number, fontSize: number,
): void {
  ...
  const text = createInsideTextElement(g, cx, cy, fontSize)

  if (showLabel) {
    appendLabelTspan(text, cx, d.label, showValue, textColor)
  }
  if (showValue) {
    appendValueTspan(text, d, cx, showLabel ? '1.2em' : '0em', textColor, 0.85)
  }
}
...
export function renderInsideArcLabels(
  ...
  for (const d of data) {
    ...
    if (span >= MIN_INSIDE_ANGLE) {
      renderInsideLabel(g, d, centroidR, fontSize)
    }
  }
}

function classifySlices(
  data: ArcLabelDatum[], centroidR: number,
): { insideData: ArcLabelDatum[], outsideData: ArcLabelDatum[] } {
...
export function renderAutoArcLabels(
  ...
  const centroidR = (innerRadius + outerRadius) / 2
  const { insideData, outsideData } = classifySlices(data, centroidR)
