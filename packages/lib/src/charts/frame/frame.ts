    ...
    this.layer('footerRightItems', footerRight, {
      ...
      events: {
        ...
        enter: (sel: any) => {
          ...
          sel.filter((d: FooterItem) => !!d.target)
            .attr('target', (d: FooterItem) => d.target!)
          sel.filter((d: FooterItem) => !!d.rel)
            .attr('rel', (d: FooterItem) => d.rel!)
        },
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        merge: (sel: any) => {
          sel
...
function buildFooterRightItems(): FooterItem[] {
  return [{
    tag: 'a',
    className: 'bc-frame-credit',
    text: 'Blueprint Chart',
    html: CREDIT_LOGO_SVG + 'Blueprint Chart',
...
export function createFrame(
  ...
  chart.draw({
    ...
    footerRightItems: buildFooterRightItems(),
  })
