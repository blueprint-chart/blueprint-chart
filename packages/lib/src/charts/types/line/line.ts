...
export function render(
  ...
  const plotted = lineData.filter(d => Number.isFinite(d.value))
  ...
  const defined = (d: LineDatum) => Number.isFinite(d.value)
  const areaGen = d3.area<LineDatum>().curve(curve).defined(defined).x(d => xPos(d)).y0(height).y1(d => y(d.value) as number)
  const lineGen = d3.line<LineDatum>().curve(curve).defined(defined).x(d => xPos(d)).y(d => y(d.value) as number)
  ...
  // the marks (above), legend and axis move in lockstep. Same-type only. The clip
  // size carries the 1px edge inflation (matches ensureClipPath above).
  const plotRect: PlotRect = { left: margin.left, top: margin.top, width: width + 2, height: height + 2 }
  tweenPlotFrame(orch, {
    svg,
    clipId,
    group: chartArea,
  ...
  // proximity overlay keep their z-order above it.
    ...
    const labelPos = (d: LineDatum) => clampPointLabel(String(d.value), xPos(d), y(d.value) as number, { width, height, margin })
  ...
  setCachedChart(container, { chartType: 'line', margin, plotRect })
