...
function readPosition(properties: PropertyNode[], key: string): number | string | undefined {
  ...
  if (node.isPercentage) return node.value as number
  if (typeof node.value === 'string') return node.value // e.g. "150px"
...
function readMaxWidth(properties: PropertyNode[]): number | string | undefined {
  ...
  if (!node) return undefined
  if (node.isPercentage) return `${node.value}%`
      ...
      if (ast.annotations) {
        ...
        config.annotations.value = ast.annotations.map((a): AnnotationConfig => {
          ...
          if (kind === 'free') {
            ...
            if (aProps.has('textOutline')) result.textOutline = toBool(aProps.get('textOutline'))
            return result
          }
          ...
          if (aProps.has('textOutline')) result.textOutline = toBool(aProps.get('textOutline'))
          if (aProps.has('showLine')) result.showLine = toBool(aProps.get('showLine'))
          ...
          if (aProps.has('showArrow')) result.showArrow = toBool(aProps.get('showArrow'))
          if (aProps.has('lineTargetDistance')) result.lineTargetDistance = Number(aProps.get('lineTargetDistance'))
          if (aProps.has('showCircle')) result.showCircle = toBool(aProps.get('showCircle'))
