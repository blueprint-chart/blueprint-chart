...
**Blueprint Chart is the open chart format AI writes and the browser renders.** Describe a chart in a compact, human-readable DSL (`.bpc`) — or let an LLM emit it — and it renders anywhere, with no backend, fully self-contained. A best-in-class editor is there for when a human wants to refine it by hand.
