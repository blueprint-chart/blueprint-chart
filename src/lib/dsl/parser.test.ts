...
describe('parser', () => {
  ...
  describe('error handling', () => {
    it('throws on missing chart keyword', () => {
      expect(() => parse('foo {}')).toThrow(/Expected keyword 'chart'/)
    })

    it('throws on missing opening brace', () => {
      expect(() => parse('chart bar }')).toThrow(/Expected lbrace/)
    })

    it('throws on missing closing brace', () => {
      expect(() => parse('chart bar { title = "x"')).toThrow(/Expected property key but got eof/)
    })

    it('throws on missing equals in property', () => {
      expect(() => parse('chart bar { title "x" }')).toThrow(/Expected equals/)
    })

    it('throws on invalid property value', () => {
      expect(() => parse('chart bar { title = { } }')).toThrow(/Expected property value/)
    })

    it('throws on missing highlight target', () => {
      expect(() => parse('chart bar { highlight { } }')).toThrow(/Expected string/)
    })

    it('throws on missing step name', () => {
      expect(() => parse('chart bar { step { } }')).toThrow(/Expected string/)
    })
